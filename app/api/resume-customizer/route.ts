...
const createResumeCustomizationPrompt = (profileData: any) => {
  if (!profileData) {
    return "Customize a resume for a senior technology leader with 25+ years experience."
  }

  return `You are an expert resume writer specializing in technology executive resumes. You're customizing a resume for Ravi Poruri based on a specific job description.
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Fallback customized resume
      const fallbackResume = {
        summary: "Technology leader with 25+ years driving digital transformations and scaling organizations. Proven track record delivering $3.2B+ revenue impact across Yahoo, Dropbox, Cisco, and current AI ventures. Expert in data platforms, AI/ML, and building world-class engineering teams from startup to IPO.",
        keyAchievements: [
          "Grew Cisco CX Cloud from MVP to $500M+ ARR in 4 years",
          "Doubled Dropbox revenue from $850M to $1.8B, led pre-IPO to IPO transition",
          "Generated $2B+ annual revenue at Yahoo managing 400B+ events daily",
          "Built and scaled engineering organizations totaling 500+ people globally"
        ],
        relevantExperience: [
        ...
        recommendations: [
          "Emphasize recent AI/ML work at Equiti Ventures to show cutting-edge technical leadership",
          "Highlight specific revenue impact numbers ($3.2B+) to demonstrate business value delivery",
          "Showcase experience scaling teams and platforms across different growth stages"
        ]
      }
